...
path_grid = mp_grid_create(0, 0, w, h, CELL_WIDTH, CELL_HEIGHT);
	...
	for (var i = 0; i < w - 1; i++) {
		if ( (i < c_room_lower_i || i > c_room_higher_i) || (j > c_room_lower_j || j < c_room_higher_j) )
			if (board[# i, j] == FLOOR) {		
				...
				// add cell to path grid; a cell means you can't go there
...
for (var j = 0; j < floor_counter; j++) {
	if (monster_odds = irandom(monster_odds)) {
		...
		obj_master_controller.enemy_count++; // increase count of enemy
		monster_odds += 5;
		max_monsters--;
		if (max_monsters < 1) { break; }
	}
	else monster_odds--;
}

obj_master_controller.level_made = true;
